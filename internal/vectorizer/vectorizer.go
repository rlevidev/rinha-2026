...
type Transaction struct {
	Transaction struct {
		Amount       float32 `json:"amount"`       // Valor total da transação. É usado na dimensão v[0] (normalizado pelo MaxAmount)
		Installments int     `json:"installments"` // Quantidade de parcelas. É usado na dimensão v[1]
		RequestedAt  string  `json:"requested_at"` // Data e hora da transação. Essencial para extrair a hora do dia (v[3]) e o dia da semana (v[4])
	} `json:"transaction"`
	...
	Terminal struct {
		KmFromHome  float64 `json:"km_from_home"` // Distância do local da transação até a casa do cliente. Usado na dimensão v[7]
		IsOnline    bool    `json:"is_online"`    // Identifica se é uma compra online ou física. Usado na dimensão v[9]
		CardPresent bool    `json:"card_present"` // Identifica se o cartão físico estava presente. Usado na dimensão v[10]
	} `json:"terminal"`

	// Histório recente
	LastTransaction *struct {
		Timestamp     string  `json:"timestamp"`       // Data da última transação. Comparado com RequestedAt para calcular a diferença em minutos (v[5])
		KmFromCurrent float64 `json:"km_from_current"` // Distância da localização da transação anterior para a atual. Usado na dimensão v[6]
	} `json:"last_transaction"`

	// Informações do estabelecimento
	Merchant struct {
		ID        string  `json:"id"`         // Identificador único do estabelecimento. Comparado com a lista KnowMerchants do cliente.
		MCC       string  `json:"mcc"`        // Merchant Category Code. Usado para buscar o nível de risco (v[12]) no mapa MccRisk
		AvgAmount float64 `json:"avg_amount"` // Valor médio das transações do estabelecimento. Usado na dimensão v[13] (normalizado pelo MaxMerchantAvg)
	} `json:"merchant"`
}
...
func (n *Normalizer) Vectorize(tx *Transaction) [14]float32 {
	...
	requestedAt, _ := time.Parse(time.RFC3339, tx.Transaction.RequestedAt) // Converte a string de data para um objeto TIme do Go.
	...
	v[0] = clamp(float32(tx.Transaction.Amount) / n.MaxAmount)             // Valor bruto normalizado pelo valor máximo observado
	v[1] = clamp(float32(tx.Transaction.Installments) / n.MaxInstallments) // Número de parcelas normalizadas pelo máximo observado

	// Proteção contra divisão por zero
	if tx.Customer.AvgAmount > 0 {
		// Ratio: quão fora do padrão é este gasto?
		v[2] = clamp(float32(tx.Transaction.Amount/tx.Customer.AvgAmount) / n.AmountVsAvgRatio)
	}
